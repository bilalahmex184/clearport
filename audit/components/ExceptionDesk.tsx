...
import { useClearPort, Exception } from '../context/ClearPortContext';
import { AlertCircle, CheckCircle2, ChevronRight, CornerDownRight, Eye, RefreshCw, XCircle, FileText, ZoomIn, ZoomOut, RotateCw, Sparkles, Undo2 } from 'lucide-react';
...
export default function ExceptionDesk() {
  ...
  const [prevExceptionId, setPrevExceptionId] = React.useState('');

  // Sync edit value synchronously during render when selection changes
  if (selectedException && selectedException.id !== prevExceptionId) {
    setPrevExceptionId(selectedException.id);
  ...
  if (!selectedEntry) {
    return (
      <div id="exception-desk-empty" className="h-[calc(100vh-140px)] flex flex-col items-center justify-center text-gray-500 font-sans">
  ...
  const sortedExceptions = [...filteredExceptions].sort((a, b) => {
    if (sortBy === 'severity') {
      // High severity (lowest confidence) first
  ...
  const highConfidenceCount = exceptions.filter((ex) => ex.status === 'Unresolved' && ex.confidence >= 70).length;
  ...
  return (
    <div id="exception-desk-root" className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-[calc(100vh-120px)] overflow-hidden font-sans">
      
      {/* LEFT PANEL: EXCEPTION LIST */}
      <div id="exception-desk-left" className="lg:col-span-3 bg-[#0c0d12] border border-gray-900 rounded-xl flex flex-col overflow-hidden">
        ...
        <div className="p-3 border-b border-gray-900 bg-[#090a0f] space-y-2.5">
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            ...
            <select
              ...
              className="bg-black border border-gray-850 text-gray-300 rounded px-1.5 py-0.5 text-xs focus:outline-none focus:border-gray-700 font-medium"
            >
          ...
          {highConfidenceCount > 0 && (
            ...
            >
              ...
              Accept {highConfidenceCount} Fields &gt; 70% Conf
            </button>
          )}
        </div>

        {/* Exceptions Scroll List */}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-900/30 text-[10px] text-gray-500 font-mono">
                    ...
                    <span className="text-gray-600">ID: {ex.id.toUpperCase()}</span>
                  </div>
                </motion.div>
              );
            })}
      ...
      {/* CENTER PANEL: INTERACTIVE DOCUMENT VIEWER */}
      <div id="exception-desk-center" className="lg:col-span-5 bg-[#0c0d12] border border-gray-900 rounded-xl flex flex-col overflow-hidden relative">
        {/* Document Tabs */}
        <div className="px-4 py-2 bg-[#0e1017] border-b border-gray-900 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-1.5">
            {['Commercial Invoice', 'Packing List', 'Bill of Lading', 'Certificate of Origin'].map((doc) => {
              // Only render tabs that exist in the documents
              const isAvailable = doc === 'Commercial Invoice' || 
                                  doc === 'Packing List' || 
                                  (selectedEntry.id === 'SHIP-2026-8802' && doc === 'Bill of Lading') ||
                                  (selectedEntry.id === 'SHIP-2026-9041' && doc === 'Certificate of Origin');
              ...
              return (
                ...
                >
                  {doc === 'Commercial Invoice' ? 'Invoice' : doc === 'Packing List' ? 'Packing List' : doc}
                </button>
              );
            })}
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 text-[8px] font-sans border-b border-gray-200 pb-2 mb-3">
                <div>
                  ...
                  <span className="text-gray-600 block leading-tight mt-0.5">
                    {selectedEntry.id === 'SHIP-2026-8802' ? '7-3 Marunouchi, Chiyoda-ku, Tokyo, JP' : 'Building 12, High-Tech Zone, Shanghai, CN'}
                  </span>
                </div>
                <div>
                  ...
                  <span className="text-gray-600 block leading-tight mt-0.5">
                    1400 Aviation Blvd, Los Angeles, CA 90045, USA
                  </span>
                </div>
              </div>
              ...
              <div className="text-[8px] font-sans space-y-2">
                <div className="grid grid-cols-4 border-b border-gray-200 pb-1 font-bold text-gray-500 uppercase tracking-widest text-[6px]">
                  ...
                  <div>PORT CODE</div>
                  <div>CARRIER</div>
                  <div className="text-right">COUNTRY OF ORIGIN</div>
                </div>
                <div className="grid grid-cols-4 border-b border-gray-100 pb-1.5 font-medium text-gray-900 font-mono">
                  <div>{selectedEntry.id === 'SHIP-2026-8802' ? 'INV-8802-AP' : 'VND-9041-SH'}</div>
                  <div>{selectedEntry.id === 'SHIP-2026-8802' ? 'LAX-2720' : 'TAC-3001'}</div>
                  <div>{selectedEntry.id === 'SHIP-2026-8802' ? 'PACIFIC AIR' : 'VANGUARD SH'}</div>
                  <div className="text-right">
                    {selectedEntry.id === 'SHIP-2026-8802' ? 'JP' : 'CN (DECLARED)'}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-2.5 text-[8.5px] text-gray-800 leading-relaxed font-serif">
                ...
                {/* Specific line item details containing values */}
                <div className="border border-gray-250 p-2.5 rounded bg-gray-50/50 mt-3 font-sans space-y-2">
                  ...
                  <div className="flex justify-between items-center text-gray-950 font-medium">
                    <span>Titanium Fasteners M12 (Grade 5 Aerospace-Spec)</span>
                    <span className="font-mono text-gray-900">
                      {selectedEntry.id === 'SHIP-2026-8802' ? '8108.90.3060' : '8504.40.9580'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-t border-gray-100 pt-1 text-gray-950">
                    <span className="text-gray-500 text-[7px]">Gross/Net Declared Weights:</span>
                    <span className="font-mono font-medium text-gray-900">
                      {selectedEntry.id === 'SHIP-2026-8802' ? '12,450 lbs (Declared)' : '4,120 lbs'}
                    </span>
                  </div>
                </div>

                <div className="pt-2 text-[7.5px] text-gray-500 leading-normal italic">
                  * Note: Discrepancy values are highlighted dynamically on OCR workspace overlays. This copy has been parsed securely through the AI extraction engine.
                </div>
              </div>
            </div>

            {/* Document Bottom Section with total value */}
            <div className="border-t-2 border-gray-900 pt-3 flex justify-between items-end font-sans">
              ...
              <div className="text-right">
                ...
                <span className="text-xs font-mono font-extrabold text-gray-950 tracking-tight block">
                  {selectedEntry.id === 'SHIP-2026-8802' ? '$128,450.00' : '$84,120.00'}
                </span>
              </div>
            </div>

            {/* Pulsing Highlight Box Overlay */}
            {selectedException && activeDocTab === selectedException.docType && (
              ...
              >
                <div className="absolute -top-3.5 left-0 text-[7px] font-mono text-white bg-amber-600 px-1 py-0.5 rounded shadow whitespace-nowrap uppercase tracking-wider font-bold">
                  FLAGGED FOR {selectedException.fieldName.toUpperCase()}
                </div>
              </motion.div>
            )}
          </div>
      ...
      <div id="exception-desk-right" className="lg:col-span-4 bg-[#0c0d12] border border-gray-900 rounded-xl flex flex-col overflow-hidden">
        ...
        {selectedException ? (
          <div className="flex-1 flex flex-col justify-between overflow-hidden">
            ...
            <div className="p-4 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="border-b border-gray-900 pb-3">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded border ${
                    selectedException.status === 'Unresolved' ? getConfidenceColor(selectedException.confidence) : 'bg-gray-950 text-gray-400 border-gray-900'
                  }`}>
                    {selectedException.status === 'Unresolved' ? getConfidenceBadge(selectedException.confidence) : selectedException.status.toUpperCase()}
                  </span>
                </div>
              ...
              {/* Plain English Discrepancy Reason */}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-black border border-gray-900 p-3 rounded-lg">
                  ...
                  <span className="text-sm font-mono font-bold text-gray-200 block mt-1">
                    {selectedException.originalValue}
                  </span>
                ...
                <div className="bg-black border border-gray-900 p-3 rounded-lg">
                  ...
                  <span className="text-sm font-mono font-bold text-gray-400 block mt-1">
                    {selectedException.fieldKey === 'netWeight' ? '14,250 lbs' : 
                     selectedException.fieldKey === 'htsCode' ? '8108.90.3030' : 
                     selectedException.fieldKey === 'countryOfOrigin' ? 'TW' : 'Verify manually'}
                  </span>
                  <span className="text-[9px] font-mono text-gray-600 block mt-0.5">
                    Source: {selectedException.fieldKey === 'netWeight' ? 'Packing List' : 
                            selectedException.fieldKey === 'htsCode' ? 'Packing List' : 
                            selectedException.fieldKey === 'countryOfOrigin' ? 'Certificate' : 'N/A'}
                  </span>
                </div>
              </div>
            ...
            <div className="p-3 bg-black/40 border-t border-gray-900 flex justify-between items-center text-[10px] text-gray-500 font-mono select-none">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[11px] text-gray-400 hover:text-white hover:bg-gray-900 border border-gray-850 px-2 py-1 rounded transition-all cursor-pointer font-bold uppercase"
              >
