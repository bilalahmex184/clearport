...
import { CheckCircle2, ChevronRight, FileSpreadsheet, AlertCircle, Calendar, ArrowUpRight, HelpCircle, FileText } from 'lucide-react';
...
export default function EntryDetailView() {
  ...
  if (!selectedEntry) {
    return (
      <div id="entry-detail-empty" className="h-[calc(100vh-140px)] flex flex-col items-center justify-center text-gray-500">
  ...
  // Horizontal Timeline Step Finder
  ...
  return (
    <div id="entry-detail-root" className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-[calc(100vh-120px)] overflow-hidden font-sans">
      
      {/* LEFT COLUMN: ACTIVE SHIIPMENTS SELECTOR & QUICK INFOS (3 cols) */}
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border ${
                    ent.status === 'Approved' ? 'bg-emerald-950 text-emerald-400 border-emerald-900/40' : 'bg-amber-950 text-amber-400 border-amber-900/40'
                  }`}>
      ...
      {/* RIGHT COLUMN: MAIN FIELDS TABLE & STATUS TIMELINE (9 cols) */}
      <div className="lg:col-span-9 bg-[#0c0d12] border border-gray-900 rounded-xl flex flex-col overflow-hidden">
            ...
            <button
              ...
              className={`flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-lg border transition-all uppercase tracking-wider ${
                isExportEnabled
                  ...
                  : 'bg-gray-950 text-gray-650 border-gray-900 cursor-not-allowed'
              }`}
            >
              <FileSpreadsheet className="w-4 h-4" />
              <span>Export to CSV Logs</span>
            </button>
          </div>
          ...
          <div className="relative pt-1 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-5 gap-2">
              ...
              <div className="text-center">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center font-mono text-xs font-extrabold z-10 transition-all ${getStepClass(1)}`}>
                  01
                </div>
                <span className="text-[10px] font-bold text-gray-400 font-mono tracking-wider block mt-2">UPLOADED</span>
              </div>

              {/* Step 2: Processed */}
              <div className="text-center">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center font-mono text-xs font-extrabold z-10 transition-all ${getStepClass(2)}`}>
                  02
                </div>
                <span className="text-[10px] font-bold text-gray-400 font-mono tracking-wider block mt-2">OCR PARSED</span>
              </div>

              {/* Step 3: Under Review */}
              <div className="text-center">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center font-mono text-xs font-extrabold z-10 transition-all ${getStepClass(3)}`}>
                  03
                </div>
                <span className="text-[10px] font-bold text-gray-400 font-mono tracking-wider block mt-2">UNDER REVIEW</span>
              </div>

              {/* Step 4: Approved */}
              <div className="text-center">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center font-mono text-xs font-extrabold z-10 transition-all ${getStepClass(4)}`}>
                  04
                </div>
                <span className="text-[10px] font-bold text-gray-400 font-mono tracking-wider block mt-2">APPROVED</span>
              </div>

              {/* Step 5: Exported */}
              <div className="text-center">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center font-mono text-xs font-extrabold z-10 transition-all ${getStepClass(5)}`}>
                  05
                </div>
                <span className="text-[10px] font-bold text-gray-400 font-mono tracking-wider block mt-2">EXPORTED</span>
              </div>
        ...
        {/* CSV export lock message banner */}
        {!isExportEnabled && (
          <div className="p-4 bg-[#140b0b] border-t border-gray-900 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-red-400 font-semibold leading-normal">
              ...
              <span>EXPORT TO SYSTEMS SUSPENDED: Resolve the {unresolvedCount} active document discrepancies above before customs clearance filing.</span>
            </div>
            <button
